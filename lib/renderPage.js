...
class RenderPage {
    ...
    async renderPage(meta) {
        let {route, routes, props, manifest, commonBuildMeta, srrPath, compliedPages, configSeo} = meta

        try {
            if (route?.path?.includes("/:")) return ""

            const component = await import(path.resolve(srrPath));
            const {default: componentFn, getStaticPaths, generateMetadata} = component;

            if (typeof getStaticPaths === "function") {
                console.log(chalk.green("Loading getStaticPaths.."))
                let data;
                try {
                    data = await getStaticPaths?.()
                } catch (ex) {
    ...
    async generateDynamicPage(meta) {
        ...
        const content = renderToString(React.createElement(componentFn, {name: "sdjlk;f"}));

        const html = this.createHTMLDocument(content, props, {route, manifest, commonBuildMeta, configSeo})

        let {filePath, routeDir} = this.generateFilePath(route, props)
