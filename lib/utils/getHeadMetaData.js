...
function genTags(head) {
    return head?.map(tag => {
        const { type, content, ...attrs } = tag;

        // Convert attributes to string safely
        const attrString = Object.entries(attrs)
            .filter(([_, value]) => value !== undefined && value !== null && value !== false)
            .map(([key, value]) => {
                if (value === true) return key; // handle boolean attributes like "disabled", "crossorigin"
  ...
  function getHeadMetaData(content, configSeo, routeData = {}) {
    ...
    const configSeoData = configSeo?.seo ?? {}
      ...
      const headTags = genTags(configSeo?.head || [])
