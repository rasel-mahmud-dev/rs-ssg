...
class RsSSGCli {
    ...
    constructor() {
        ...
        loadConfig(process.cwd(), path.resolve(path.dirname(__dirname), '.')).then(config => {
            this.config["configSeo"] = config;
            this.config["buildDir"] = config?.outDir || ".rs-ssg"
        }).catch(ex=>{
            console.log(`Failed to load configuration ${chalk.red(ex)}`);
        })
