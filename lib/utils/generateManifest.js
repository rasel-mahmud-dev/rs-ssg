...
export async function getManifest(metafile, buildResult) {
    ...
    const getRelativePath = (fullPath, buildDir = "dist") => {
        // return fullPath.replace(/^.rs-ssg\/client\//, '').replace(/^.rs-ssg\//, '');

        return fullPath.replace(new RegExp(`^${buildDir}/client/`), '')
            .replace(new RegExp(`^${buildDir}/`), '');
    ...
    for (const [outputPath, outputInfo] of Object.entries(metafile.outputs)) {
        if (outputPath.endsWith('.css')) {
            ...
            if (entryPoint) {
                ...
                for (const [_, otherInfo] of Object.entries(metafile.outputs)) {
...
export async function generateManifest(metafile, buildResult, buildDir = "dist") {
    const manifest = await getManifest(metafile, buildResult)
    await fs.writeFile(`${buildDir}/manifest.json`, JSON.stringify(manifest, null, 2));
}