...
const markdownPlugin = {
    ...
    setup(build) {
        build.onResolve({ filter: /\.md$/ }, (args) => {
            return {
                path: args.path,
                namespace: 'markdown-ns', // custom namespace
            };
        });

        build.onLoad({ filter: /\.md$/, namespace: 'markdown-ns' }, async (args) => {
            const contents = await fs.promises.readFile(args.path, 'utf8');
            return {
                contents: `export default ${JSON.stringify(contents)};`,
                loader: 'js', // treat it as JS
            };
        });
    },
};
...
class Builder {
    ...
    async build(routes) {
        ...
        await generateManifest(buildResult.metafile, buildResult, this.config.buildDir);
